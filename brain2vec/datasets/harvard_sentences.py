...
class HarvardSentences(BaseASPEN):
    ...
    default_base_path = environ.get(
        env_key, path_map.get(socket.gethostname(), default_hvs_path)
    )
    mat_d_keys = dict(
        signal="sEEG_signal",
        signal_fs="fs_signal",
    ...
    def make_pipeline_map(self, default="audio_gate"):
        ...
        audio_gate_steps = [
            (
                "Threshold",
                ps.PowerThreshold(
                    speaking_window_samples=48000 // 16,
            ...
            (
                "speaking_indices",
                ps.WindowSampleIndicesFromIndex(
                    "stim_pwrt",
        ...
        start_stop_steps = [
            ("new_mtss", ps.AppendExtraMultiTaskStartStop()),
            # Stims from Start-stop-times
            (
                "speaking_word_stim",
                ps.NewStimFromRegionStartStopTimes(
        ...
        p_map = {
            ...
            "audio_gate_all_region": Pipeline(
                parse_arr_steps
                + parse_input_steps
                + start_stop_steps
                + audio_gate_all_region_steps
                ...
                + [
                    ...
                    (
                        ...
                        ps.WindowSampleIndicesFromStim(
                            "speaking_region_stim", stim_value_remap=0, **region_kws
                        ),
                    ),
                    (
                        "listening_indices",
                        ps.WindowSampleIndicesFromStim(
                            "listening_region_stim", stim_value_remap=1, **region_kws
                        ),
                    ),
                    (
                        "mouthing_indices",
                        ps.WindowSampleIndicesFromStim(
                            "mouthing_region_stim", stim_value_remap=2, **region_kws
                        ),
                    ),
                    (
                        "imagining_indices",
                        ps.WindowSampleIndicesFromStim(
                            "imagining_region_stim", stim_value_remap=3, **region_kws
                        ),
                    ),
                    ("output", "passthrough"),
                ]
            ),
            "region_classification_from_word_stim": Pipeline(
                parse_arr_steps
                + parse_input_steps
                + start_stop_steps
                + [
            ...
            "audio_gate_speaking_only": Pipeline(
                parse_arr_steps
                + parse_input_steps
                + start_stop_steps
    ...
    def make_filename(cls, patient, session, trial, location):
        ...
        if loc_map is None:
            raise KeyError(
                f"Got location {location}, but valid locations are: {list(cls.all_patient_maps.keys())}"
            )
